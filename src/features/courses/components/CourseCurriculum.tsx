...
export function CourseCurriculum({ modules }: CourseCurriculumProps) {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-xl font-bold text-foreground mb-6">Plan de Estudios</h3>
          ...
          return (
            <div
              ...
              className={`border border-border/60 rounded-2xl overflow-hidden transition-all duration-300 ${
                isOpen ? "bg-card shadow-lg ring-1 ring-primary/20" : "bg-card/50 hover:bg-card hover:border-border"
              }`}
            >
              {/* Header */}
              <button
                    ...
                    <div className="flex flex-col gap-1 px-3 ml-8 border-l border-border/50">
                      {module.lessons.length === 0 ? (
                        ...
                        module.lessons.map((lesson) => (
                          <div
                            key={lesson.id}
                            className="flex items-center gap-3 p-3 ml-2 rounded-xl hover:bg-muted/50 transition-colors group cursor-pointer"
                          >
